...
func TestWithdraw(t *testing.T) {
	...
	wg.Wait()
	assert.Equal(t, 39.3, btcwallet.GetBalance(), "Withdraw safety for concurrency useage")
	wg.Add(1)
	go func(value float64, wg *sync.WaitGroup) {
		defer wg.Done()
		btcwallet.Withdraw(value, wg)
	...
	wg.Wait()
	assert.Equal(t, 39.3, btcwallet.GetBalance(), "Validation negative numbers")
	wg.Add(1)
	go func(value float64, wg *sync.WaitGroup) {
		defer wg.Done()
		btcwallet.Withdraw(value, wg)
	...
	wg.Wait()
	assert.Equal(t, 39.3, btcwallet.GetBalance(), "Validation zero number")
